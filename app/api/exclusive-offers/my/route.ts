...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from('products')
      .select('id, name, description, points_price, created_at, status, is_active, category:category_id(id, name, game:game_id(id, name))')
    ...
    const packs = (data ?? []).map((pack: any) => ({
      ...
      game: pack.category?.game
        ? {
            id: pack.category.game.id,
            name: pack.category.game.name,
          }
        : null,
    }))
