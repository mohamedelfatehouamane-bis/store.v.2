...
export async function POST(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('products')
      .update({ status: 'approved', approved_by: authResult.auth.id, approved_at: new Date().toISOString() })
      .eq('id', id)
      .eq('status', 'pending')
      .select('*')
      .single()
    ...
    if (error) {
      ...
      return NextResponse.json({ error: error.message || 'Unable to approve product' }, { status: 500 })
    }
    ...
    console.error('Approve product unexpected error:', error)
